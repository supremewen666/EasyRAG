"""Built-in single-node storage backends for EasyRAG's EasyRAG subsystem."""
...
from easyrag.rag.knowledge.extraction import summarize_entity_descriptions
...
from easyrag.rag.utils import dedupe_strings
...
def _merge_owner_maps(
    ...
    for doc_id, payload in incoming.items():
        current = merged.setdefault(
            str(doc_id), {"count": 0, "types": [], "descriptions": []}
        )
        current["count"] = int(current.get("count", 0)) + int(payload.get("count", 0))
        current["types"] = sorted(
            set(list(current.get("types", [])) + list(payload.get("types", [])))
        )
        current["descriptions"] = dedupe_strings(
            [
                str(value).strip()
                for value in list(current.get("descriptions", []))
                + list(payload.get("descriptions", []))
                if str(value).strip()
            ]
        )
    return merged
...
def _refresh_entity_node(node: dict[str, Any]) -> dict[str, Any]:
    ...
    base_entity_types = [
        ...
        for entity_type in list(node.get("manual_entity_types", []))
        if str(entity_type).strip()
    ]
    if not base_entity_types:
        base_entity_types = [
            str(entity_type).strip()
            for entity_type in list(node.get("entity_types", []))
            if str(entity_type).strip()
        ]
    ...
    node["owner_count"] = sum(
        int(payload.get("count", 0)) for payload in owners.values()
    )
    return node
...
def _merge_node_payload(
    ...
    if merged.get("kind") == "entity":
        ...
        incoming_manual_description = (
            str(incoming.get("manual_description", "")).strip()
            or str(incoming.get("description", "")).strip()
        )
        existing_manual_description = str(
            existing.get("manual_description", "")
        ).strip()
        merged["manual_description"] = (
            incoming_manual_description or existing_manual_description
        )
        merged["aliases"] = dedupe_strings(
            [
                str(value).strip()
                for value in list(existing.get("aliases", []))
                + list(incoming.get("aliases", []))
                if str(value).strip()
            ]
        )
        merged["provenance"] = dedupe_strings(
            [
                str(value).strip()
                for value in list(existing.get("provenance", []))
                + list(incoming.get("provenance", []))
                if str(value).strip()
            ]
        )
        merged["metadata"] = {
            **dict(existing.get("metadata", {})),
            **dict(incoming.get("metadata", {})),
        }
...
def _merge_contributions(
    ...
    for doc_id, payload in incoming.items():
        current = merged.setdefault(
            str(doc_id),
            {"weight": 0.0, "relations": [], "descriptions": [], "chunk_ids": []},
        )
        current["weight"] = float(current.get("weight", 0.0)) + float(
            payload.get("weight", 0.0)
        )
        current["relations"] = sorted(
            set(list(current.get("relations", [])) + list(payload.get("relations", [])))
        )
        current["descriptions"] = dedupe_strings(
            [
                str(value).strip()
        ...
        current["chunk_ids"] = sorted(
            set(list(current.get("chunk_ids", [])) + list(payload.get("chunk_ids", [])))
        )
    return merged
...
def _refresh_contributed_edge(edge: dict[str, Any]) -> dict[str, Any]:
    ...
    edge["weight"] = sum(
        float(payload.get("weight", 0.0)) for payload in contributions.values()
    )
...
def _merge_edge_payload(
    ...
    merged = {**existing, **incoming}
    merged["weight"] = float(existing.get("weight", 0.0)) + float(
        incoming.get("weight", 0.0)
    )
    merged["relations"] = sorted(
        set(
            list(existing.get("relations", []))
            + list(incoming.get("relations", []))
            + [str(incoming.get("relation", "")).strip()]
        )
        - {""}
    )
    return merged
...
def _is_document_owned_node(
    node_id: str, node: dict[str, Any], document_id: str
) -> bool:
    """Return whether a graph node is owned by one document."""

    if (
        node_id == document_id
        or node_id == f"summary::{document_id}"
        or node_id.startswith(f"{document_id}::chunk::")
    ):
        return True
    if str(node.get("document_id", "")) == document_id:
        return True
    if str(node.get("doc_id", "")) == document_id and node.get("kind") in {
        "document",
        "summary",
        "chunk",
    }:
...
def _normalize_relation_record(relation: dict[str, Any]) -> dict[str, Any]:
    ...
    provenance = dedupe_strings(
        [
            str(value).strip()
            for value in list(relation.get("provenance", []))
            if str(value).strip()
        ]
    )
...
class JSONKVStorage(BaseKVStorage):
    ...
    async def delete_by_document(self, document_id: str) -> dict[str, int]:
        ...
        for summary_id in [
            ...
            if str(summary.get("document_id", "")) == document_id
            or str(summary.get("metadata", {}).get("doc_id", "")) == document_id
        ]:
    ...
    async def get_chunks(self, chunk_ids: list[str]) -> list[dict[str, Any]]:
        return [
            self._chunks[chunk_id] for chunk_id in chunk_ids if chunk_id in self._chunks
        ]
...
class TokenVectorStorage(BaseVectorStorage):
    ...
    async def initialize(self) -> None:
        ...
        self._store = {
            str(namespace): {
                str(item_id): dict(payload) for item_id, payload in dict(bucket).items()
            }
    ...
    async def similarity_search(
        ...
        for item in bucket.values():
            ...
            phrase_bonus = (
                1 if query.lower() in str(item.get("text", "")).lower() else 0
            )
...
class EmbeddingVectorStorage(BaseVectorStorage):
    ...
    async def initialize(self) -> None:
        ...
        for json_path in self._root.glob("*.json"):
            ...
            self._records[namespace] = {
                str(record["id"]): record for record in ordered_records
            }
            ...
            if npy_path.exists():
                ...
                if matrix.ndim == 2 and len(ordered_records) == matrix.shape[0]:
                    for index, record in enumerate(ordered_records):
                        embedding_bucket[str(record["id"])] = np.array(
                            matrix[index], dtype=np.float32
                        )
        ...
        for namespace, bucket in self._records.items():
            ...
            if records and embedding_bucket:
                dimension = next(
                    (
                        int(vector.shape[0])
                        for vector in embedding_bucket.values()
                        if vector.ndim == 1
                    ),
                    0,
                )
    ...
    async def upsert(self, namespace: str, items: list[dict[str, Any]]) -> None:
        ...
        for item in items:
            payload = dict(item)
            payload["tokens"] = payload.get("tokens") or _tokenize(
        ...
        if embeddings.size and embeddings.shape[0] == len(changed_payloads):
            for index, payload in enumerate(changed_payloads):
                embedding_bucket[str(payload["id"])] = np.array(
                    embeddings[index], dtype=np.float32
                )
            self._rebuild_hnsw_index(namespace)
            return
        ...
        if dimension > 0:
            for payload in changed_payloads:
                embedding_bucket[str(payload["id"])] = np.zeros(
    ...
    async def _embed_records(self, records: list[dict[str, Any]]) -> np.ndarray:
        ...
        try:
            inputs = [
                record.get("embedding_input", str(record.get("text", "")))
                for record in records
            ]
    ...
    def _rebuild_hnsw_index(self, namespace: str) -> None:
        ...
        try:
            embeddings = np.stack(
                [embedding_bucket[item_id] for item_id in record_ids]
            ).astype(np.float32)
        except Exception:
            return
        ...
        try:
            ...
            index.init_index(
                max_elements=len(record_ids),
                ef_construction=max(100, min(400, len(record_ids) * 4)),
                M=16,
            )
    ...
    async def _dense_similarity_search(
        ...
        try:
            ...
            if (
                ...
                or query_embedding.shape[1] != embeddings.shape[1]
            ):
                return []
            normalized_records = embeddings / np.maximum(
    ...
    async def _hnsw_similarity_search(
        self, namespace: str, query: str, top_k: int
    ) -> list[dict[str, Any]]:
        ...
        dimension = next(
            (
                int(embedding_bucket[item_id].shape[0])
                for item_id in record_ids
                if item_id in embedding_bucket
            ),
            0,
        )
        if dimension <= 0:
        ...
        try:
            ...
            labels, distances = index.knn_query(
                query_embedding, k=min(top_k, len(record_ids))
            )
        except Exception:
            return []
...
class NetworkXGraphStorage(BaseGraphStorage):
    ...
    async def initialize(self) -> None:
        ...
        self._relations = {
            str(relation_id): dict(record)
            for relation_id, record in dict(raw_relations).items()
        }
    ...
    def _iter_nodes(self) -> list[tuple[str, dict[str, Any]]]:
        ...
        return [
            (str(node_id), dict(data))
            for node_id, data in self._graph.get("nodes", {}).items()
        ]
    ...
    def _delete_node(self, node_id: str) -> None:
        ...
        for edge_key in [
            edge_key
            for edge_key, edge in graph_edges.items()
    ...
    def _iter_edges(self) -> list[tuple[str, str, dict[str, Any]]]:
        if nx is not None:
            return [
                (str(source), str(target), dict(data))
    ...
    def _relation_records_for_pair(
        self, source_entity_id: str, target_entity_id: str
    ) -> list[dict[str, Any]]:
        ...
        return [
            dict(record)
            for record in self._relations.values()
            if self._relation_pair(
                record["source_entity_id"], record["target_entity_id"]
            )
            == pair
        ]

    def _refresh_relation_edge(
        self, source_entity_id: str, target_entity_id: str
    ) -> None:
                ...
                "relations": sorted(
                    {
                        ...
                        for record in records
                        if str(record.get("relation", "")).strip()
                    }
                ),
    ...
    async def upsert_edges(self, edges: list[dict[str, Any]]) -> None:
        for edge in edges:
            ...
            existing = self._get_edge_data(source, target) or {
                "source": source,
                "target": target,
                "weight": 0.0,
                "relations": [],
    ...
    async def upsert_relation_records(
        ...
        for relation in relations:
            ...
            if existing is not None:
                affected_pairs.add(
                    self._relation_pair(
                        existing["source_entity_id"], existing["target_entity_id"]
                    )
                )
            self._relations[normalized["id"]] = normalized
            affected_pairs.add(
                self._relation_pair(
                    normalized["source_entity_id"], normalized["target_entity_id"]
                )
            )
    ...
    async def delete_by_document(self, document_id: str) -> dict[str, Any]:
        ...
        for relation_id, relation in list(self._relations.items()):
            ...
            if (
                document_id not in provenance
                and str(relation.get("metadata", {}).get("doc_id", "")).strip()
                != document_id
            ):
                continue
            removed_relation_ids.append(relation_id)
            affected_relation_pairs.add(
                self._relation_pair(
    ...
    async def resolve_entity_ids(
        ...
        for node_id, node in self._iter_nodes():
            ...
            candidates.update(
                ...
                for alias in list(node.get("aliases", []))
                if str(alias).strip()
            )
            score = 0.0
            for name in normalized_names:
                if name in candidates:
                    ...
                    name in candidate or candidate in name
        ...
        results.sort(
            key=lambda item: (
                ...
                float(item.get("owner_count", 0.0)),
            ),
            reverse=True,
        )
    ...
    async def merge_entities(
        ...
        for neighbor_id in self._neighbors(source_entity_id):
            ...
            if neighbor is None or edge is None:
                continue
            if (
                str(neighbor.get("kind", "")) == "entity"
                and str(edge.get("kind", "")) == "semantic_relation"
            ):
                continue
            payload = dict(edge)
            ...
            self._set_edge_data(
                target_entity_id, neighbor_id, _merge_edge_payload(existing, payload)
            )
    ...
    async def merge_relations(
        ...
        if source is None or target is None:
            return {"merged": 0}
        merged = dict(target)
        merged["weight"] = float(target.get("weight", 1.0)) + float(
            source.get("weight", 1.0)
        )
        merged["provenance"] = dedupe_strings(
            [
                str(value).strip()
                for value in list(target.get("provenance", []))
                + list(source.get("provenance", []))
                if str(value).strip()
            ]
        )
        merged["description"] = summarize_entity_descriptions(
            [
                str(target.get("description", "")).strip(),
        ...
        self._refresh_relation_edge(
            source["source_entity_id"], source["target_entity_id"]
        )
        self._refresh_relation_edge(
        ...
        *,
        ...
        for node_id in node_ids:
            for neighbor_id in self._neighbors(node_id):
                ...
                scores[neighbor_id] = scores.get(neighbor_id, 0.0) + float(
                    edge.get("weight", 1.0)
                )
        ...
        return [
            {
                "id": node_id,
                **(self._get_node_data(node_id) or {}),
