...
def build_generation_prompt(question: str, context_block: str, param: AnswerParam) -> str:
    """Build one compact answer-synthesis prompt."""

    style = param.style.strip().lower()
    if style == "extractive":
        answer_instruction = "Prefer extractive phrasing and keep the wording close to the evidence."
    elif style == "abstractive":
        answer_instruction = "Write a concise abstractive answer, but stay fully grounded in the evidence."
    else:
        answer_instruction = "Write a concise citation-aware answer grounded in the evidence."
    ...
    if param.require_citations:
        instructions.append("Use citation markers like [1] and [2] when you make a claim.")
